...
const metadataFileName = args['metadata-file'] || 'release-metadata.json'
const manifestFileName = args['manifest-file'] || 'publish-manifest.txt'
...
const metadataPath = path.join(absoluteReleaseDir, metadataFileName)
fs.writeFileSync(metadataPath, `${JSON.stringify(result.metadata, null, 2)}\n`)

const manifestPaths = buildPublishManifestPaths(absoluteReleaseDir, result.recognizedAssets, metadataFileName)
const manifestPath = path.join(absoluteReleaseDir, manifestFileName)
fs.writeFileSync(manifestPath, `${manifestPaths.join('\n')}\n`)
